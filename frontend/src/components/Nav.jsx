  ...
  return (
    <div className='w-full h-[80px] bg-[white] fixed top-0 shadow-lg flex  justify-between md:justify-around items-center px-[10px] left-0 z-[80]'>
      <div className='flex justify-center items-center gap-[10px]'>
        ...
        {searchData.length>0 && <div className='absolute top-[90px] min-h-[100px] left-0 lg:left-[20px] w-[100%] lg:w-[700px] shadow-lg bg-white flex flex-col gap-[20px] p-[20px]'>
          {searchData.map((sea)=>(
        ...
        <div className='flex justify-center items-center gap-[20px]'>
          ...
          {showPop && <div className='w-[300px] min-h-[300px] bg-white shadow-lg absolute top-[75px] rounded-lg flex flex-col items-center p-5 gap-5 right-[20px] lg:right-[100px]'>
            <div className='w-[70px] h-[70px] rounded-full overflow-hidden'>
            ...
            <button className='w-full h-[40px] rounded-full border-2 border-[#2dc0ff] text-[#2dc0ff]' onClick={()=>handleGetProfile(userData.userName)}>View Profile</button>
            ...
            <button className='w-full h-[40px] rounded-full border-2 border-[#ec4545] text-[#ec4545]' onClick={handleSignOut}>Sign Out</button>
          </div>}
          


          <div className='lg:flex flex-col items-center justify-center text-gray-600 hidden cursor-pointer' onClick={()=>navigate("/")}>
          <AiFillHome className='w-[23px] h-[23px] text-gray-600'/>
          <div>Home</div>
          </div>
          ...
          <IoIosNotifications className='w-[27px] h-[27px] text-gray-600'/>
          <div className='hidden md:block'>Notifications</div>
          </div>
          <div className='w-[50px] h-[50px] rounded-full overflow-hidden cursor-pointer' onClick={()=>setShowPop(prev=>!prev)}>
            <img src={userData.profileImage || dp} alt="" className='w-full h-full'/>
          </div>
        </div>
    </div>
