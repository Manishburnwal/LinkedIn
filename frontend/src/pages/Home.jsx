...
let {serverUrl}=useContext(authDataContext)
  ...
  return (
    <div className='w-full min-h-[100vh] bg-[#eceade] pt-[100px] flex items-center lg:items-start justify-center gap-[20px] px-[20px] flex-col lg:flex-row relative pb-[50px]'>
      {edit && <EditProfile/>}
      ...
      <div className='w-full lg:w-[25%] min-h-[200px] bg-[white] shadow-lg rounded-lg p-[10px] relative'>
        ...
        <button className='w-full h-[40px] my-[20px] rounded-full border-2 border-[#2dc0ff] text-[#2dc0ff] flex items-center justify-center gap-[10px]' onClick={()=>setEdit(true)}>Edit Profile <HiPencil /></button>
      </div>
      ...
      {uploadPost && <div className='w-[90%] max-w-[500px] h-[600px] bg-white shadow-lg top-[100px] rounded-lg fixed z-[200] p-[20px] flex items-start justify-start flex-col gap-5'>
          <div className='absolute top-[20px] right-[20px] cursor-pointer'><RxCross2 className='w-[25px] h-[25px] text-gray-800 font-bold' onClick={()=>setUploadPost(false)}/></div>
          ...
          <div className='w-full h-[200px] flex flex-col'>
            ...
            <BsCardImage className='w-[24px] h-[24px] text-gray-500 cursor-pointer' onClick={()=>image.current.click()}/>
            </div>
      ...
      <div className='w-full lg:w-[50%] min-h-[200px] bg-[#eceade] flex flex-col gap-[20px]'>
        <div className='w-full h-[120px] bg-white p-5 shadow-lg rounded-lg flex items-center justify-center gap-[10px]'>
          <div className='w-[65px] h-[65px] rounded-full overflow-hidden flex items-center justify-center cursor-pointer'>
              <img src={userData.profileImage || dp} alt="" className='h-full'/>
          </div>
        ...
        {suggestedUser.length>0 && <div className='flex flex-col gap-[10px]'>
          {suggestedUser.map((su)=>(
            <div className='flex items-center gap-[10px] mt-[10px] cursor-pointer hover:bg-gray-200 rounded-lg p-[5px]' onClick={()=>handleGetProfile(su.userName)}>
              ...
              <div>
                <div className='text-[18px] font-semibold text-gray-700'>{`${su.firstName} ${su.lastName}`}</div>
