...
export const sendConnection = async (req,res)=>{
    try {
        ...
        if(sender==id){
            return res.status(400).json({message:"you can't send request yourself"})
        }
...
export const removeConnection = async (req,res)=>{
    try {
        ...
        await User.findByIdAndUpdate(myId, { $pull: { connection:
        otherUserId } });
        await User.findByIdAndUpdate(otherUserId, { $pull: { connection:
        myId } });
